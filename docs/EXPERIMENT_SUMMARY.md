...
> 生成时间: 2026-01-23  
> 本报告总结了我们模拟用户使用 RAGEnhancedAgentMemory 插件优化自己的 Agent 进行生产环境测试的结果
...
| 指标 | 增强版 (vLLM) | 基线版 (transformers) | 差异 |
|------|---------------|----------------------|------|
...
**关键发现**：
- 🔥 **本地 vLLM 优化效果显著**：相比基线系统，延迟降低 95.5%，吞吐量提升 80.2%
- 📊 **云端 API 模式**：虽然延迟略高，但功能完整，适合快速部署
- 💡 **推荐方案**：本地部署时使用 vLLM 可以获得最佳性能
...
|---------|------------|------------|------|
| **云端 API** | 100.0% | 33.3% | +66.7 百分点 |
| **本地模型** | 100.0% | 0.0% | +100.0 百分点 |
...
| 指标 | 云端 API | 本地模型 | 说明 |
|------|---------|---------|------|
| **噪音过滤率** | 73.6% | 60.4% | 云端 API 模式下过滤更严格 |
| **信息保留率** | 100.0% | 100.0% | 两种模式下都达到 100% |
...
1. **无长期记忆**：
   - 仅使用滑动窗口（10 轮），超出窗口的对话已丢失
   - 召回成功率：**0.0% - 33.3%**（取决于部署模式）

2. **无存储优化**：
   - 噪音过滤率为 0%，所有对话都存储（在窗口内）

3. **信息丢失**：
   - 信息保留率仅 11.0%，早期关键信息无法召回

4. **推理性能**（本地 transformers 模式）：
   - 平均延迟：**8011.1 ms**（比增强版慢 22 倍）
   - 首字延迟：**800.0 ms**（比增强版慢 22 倍）
   - 吞吐量：**33.7 tokens/s**（比增强版慢 44%）
...
1. **长期记忆召回**：召回成功率 100.0%（两种模式下都达到）
2. **存储优化**：噪音过滤率 60.4% - 73.6%，有效减少存储冗余
3. **信息保留**：信息保留率 100.0%，关键信息得到有效保留
4. **推理性能**（本地 vLLM）：延迟降低 95.5%，吞吐量提升 80.2%
...
| **召回能力** |
| 增强版召回率 | 100.0% | 100.0% | 两种模式下都达到 |
| 基线版召回率 | 33.3% | 0.0% | 本地模型模式下更差 |
| **存储效率** |
| 噪音过滤率 | 73.6% | 60.4% | 云端 API 模式下过滤更严格 |
| 信息保留率 | 100.0% | 100.0% | 两种模式下都达到 |
| 存储数 | 24 | 36 | 本地模型模式下存储更多 |
