# 存储效率优化测试框架

## 📋 概述

本文档描述了存储效率优化的完整测试框架，用于验证以下功能：
1. **语义去重与低价值过滤** - 验证存储降低 45%
2. **时间加权衰减机制** - 验证检索精度提升
3. **Mock 自动化测试** - 无需真实时间等待
4. **Ragas 评估验证** - 端到端质量保证

## 🎯 测试目标

### 核心指标
- **存储降低率 ≥ 45%**：通过去重和过滤减少冗余存储
- **检索精度提升**：通过时间衰减机制提升相关性排序
- **Context Precision 提升**：高质量信息占比提高
- **Faithfulness 提升**：减少幻觉，提高回答准确性

## 📊 测试方法：实验组 vs 对照组

### 1. 语义去重与低价值过滤测试

#### 1.1 测试语料准备

生成包含大量"噪音"的对话脚本：
- **重复信息**：连续问三次相同或语义高度接近的问题
  - "如何配置 vLLM？"
  - "vLLM 怎么配？"
  - "讲讲 vLLM 的配置方法"
  
- **低价值信息**：大量寒暄对话
  - "你好"、"谢谢"、"好的"、"在吗"、"收到"等

#### 1.2 对比实验

**对照组 (Baseline)**：
- 关闭过滤逻辑，将所有对话直接存入 Qdrant
- 记录 Collection 中的点位总数（Points Count）

**实验组 (Optimized)**：
- 开启基于语义相似度（阈值 0.95）的去重
- 开启基于信息熵/分类器的低价值过滤
- 记录过滤后的存储条数

#### 1.3 指标计算

$$\text{存储降低率} = \frac{\text{对照组条数} - \text{实验组条数}}{\text{对照组条数}} \times 100\%$$

**目标**：该数值应接近 45%

**实现位置**：
- `tests/test_storage_optimization.py::TestDeduplicationAndFiltering`
- `scripts/benchmark/storage_optimization_test.py`

---

### 2. 时间加权衰减机制测试

#### 2.1 构造时间差数据

手动向 Qdrant 插入两条语义相似但时间戳不同的数据：

- **老记忆 A（1个月前）**：
  - 内容："我喜欢红色的跑车"
  - `access_count`: 10（被频繁访问，强化）
  - `timestamp`: 1个月前

- **新记忆 B（1小时前）**：
  - 内容："我最近在看蓝色的轿车"
  - `access_count`: 1（访问次数少）
  - `timestamp`: 1小时前

#### 2.2 检索验证

**查询**："我喜欢什么车？"

**验证逻辑**：
根据时间加权公式：
$$S = S_{semantic} \cdot e^{-\lambda t} \cdot (1 + \alpha \cdot \log(1 + access\_count))$$

**预期结果**：
- 如果 B 的得分高于 A：说明时间衰减生效 ✅
- 如果 A 的得分依然很高：说明记忆强化机制生效 ✅

#### 2.3 使用 Mock 进行自动化测试

**问题**：真实测试需要等待"一个月"

**解决方案**：Mock 系统时间

```python
@patch('time.time')
def test_time_decay_mechanism(self, mock_time):
    current_time = time.time()
    
    # 老记忆：30天前
    mock_time.return_value = current_time - 30 * 24 * 3600
    memory.add_memory(...)
    
    # 新记忆：1小时前
    mock_time.return_value = current_time - 3600
    memory.add_memory(...)
    
    # 检索：当前时间
    mock_time.return_value = current_time
    results = memory.search(...)
```

**实现位置**：
- `tests/test_storage_optimization.py::TestTimeWeightedDecay`

---

### 3. Ragas 评估验证

#### 3.1 Context Precision（上下文精确度）

**目标**：验证在过滤掉 45% 的"废话"后，检索回来的内容中高质量信息是否占比更高

**测试步骤**：
1. 在对照组上运行 Ragas 评估
2. 在实验组上运行 Ragas 评估
3. 对比 Context Precision 指标

**预期结果**：
- 实验组的 Context Precision > 对照组的 Context Precision

#### 3.2 Faithfulness（忠实度）

**目标**：验证减少干扰信息后，LLM 是否能更准确地基于上下文回答，减少幻觉

**测试步骤**：
1. 构造测试问题集
2. 在对照组上评估 Faithfulness
3. 在实验组上评估 Faithfulness
4. 对比结果

**预期结果**：
- 实验组的 Faithfulness > 对照组的 Faithfulness

**实现位置**：
- `tests/test_storage_optimization.py::TestRagasEvaluation`
- 集成 `src/evaluation/ragas_eval.py`

---

## 🛠️ 使用指南

### 运行存储优化测试

```bash
# 运行完整测试套件
python scripts/benchmark/storage_optimization_test.py

# 运行单元测试
pytest tests/test_storage_optimization.py -v
```

### 查看测试结果

测试结果保存在：
- `scripts/benchmark_results/storage_optimization/storage_optimization_report.json`

### 测试报告格式

```json
{
  "timestamp": 1234567890,
  "test_type": "存储效率优化测试",
  "baseline": {
    "total_memories": 20,
    "archived_count": 20,
    "input_count": 20
  },
  "optimized": {
    "total_memories": 11,
    "archived_count": 11,
    "input_count": 11,
    "filtered_count": 9
  },
  "improvements": {
    "storage_reduction_rate": 45.0,
    "storage_reduction_absolute": 9
  },
  "target_achievement": {
    "storage_reduction_45pct": true
  }
}
```

---

## 📝 实现状态

### ✅ 已实现

- [x] 基础测试框架
- [x] 测试数据生成器
- [x] 对照组/实验组对比测试
- [x] 简化版低价值过滤
- [x] Mock 时间测试框架

### ⚠️ 待实现功能

要实现完整的 45% 存储降低，需要以下功能：

1. **语义相似度去重**（阈值 0.95）
   - 当前：只有 MD5 哈希去重（完全相同的文本）
   - 需要：基于向量相似度的去重（语义相似）

2. **低价值信息过滤**
   - 当前：基于关键词的简化过滤
   - 需要：基于信息熵和分类器的智能过滤

3. **时间加权衰减机制**
   - 当前：时间戳存储，但未在检索中使用
   - 需要：实现时间衰减公式

4. **访问计数跟踪**
   - 当前：未跟踪 access_count
   - 需要：记录每次检索访问

5. **Ragas 集成**
   - 当前：测试框架已准备
   - 需要：实际运行 Ragas 评估

---

## 🔧 实现建议

### 1. 语义相似度去重

在 `src/memory/long_term.py::archive_from_short_term` 中：

```python
def archive_from_short_term(self, ..., semantic_dedup_threshold=0.95):
    # 对每个新内容：
    # 1. 计算嵌入向量
    # 2. 与已有内容计算相似度
    # 3. 如果相似度 > 0.95，跳过
```

### 2. 低价值信息过滤

在 `src/core.py::save_context` 中：

```python
def save_context(self, inputs, outputs):
    # 计算信息熵
    entropy = calculate_entropy(inputs["input"])
    
    # 分类器判断是否为低价值
    if is_low_value(inputs["input"], entropy):
        return  # 不保存
    
    # 保存到短期记忆
    self.short_term_memory.add_conversation_turn(...)
```

### 3. 时间加权检索

在 `src/memory/long_term.py::search` 中：

```python
def search(self, query, ...):
    # 获取原始相似度分数
    results = vector_search(...)
    
    # 应用时间衰减和强化
    for result in results:
        age = current_time - result.metadata["timestamp"]
        access_count = result.metadata.get("access_count", 0)
        
        # 时间衰减
        time_decay = math.exp(-lambda_param * age)
        
        # 访问强化
        reinforcement = 1 + alpha * math.log(1 + access_count)
        
        # 综合分数
        result.relevance_score *= time_decay * reinforcement
    
    # 重新排序
    results.sort(key=lambda x: x.relevance_score, reverse=True)
```

---

## 📊 预期结果

### 存储降低率

在真实的客服场景（高频重复和寒暄）中：
- **对照组**：存储 100 条记忆
- **实验组**：存储 55 条记忆（过滤 45 条）
- **存储降低率**：45% ✅

### 检索精度

- **Context Precision**：从 0.65 提升到 0.85 ✅
- **Faithfulness**：从 0.70 提升到 0.90 ✅

---

## 🔍 故障排除

### 问题 1：存储降低率不足 45%

**原因**：
- 当前实现只有基础去重
- 未实现语义相似度去重
- 低价值过滤是简化版

**解决方案**：
- 实现语义相似度去重（阈值 0.95）
- 实现智能低价值过滤

### 问题 2：时间衰减测试失败

**原因**：
- 未实现时间加权公式
- 未跟踪 access_count

**解决方案**：
- 实现时间衰减机制
- 添加访问计数跟踪

### 问题 3：Ragas 评估无法运行

**原因**：
- 依赖未安装
- API Key 未配置

**解决方案**：
- 安装 Ragas：`pip install ragas`
- 配置 API Key

---

## 📚 参考资料

- [Ragas 评估框架文档](https://docs.ragas.io/)
- [语义相似度计算](https://www.sbert.net/)
- [信息熵计算](https://en.wikipedia.org/wiki/Entropy_(information_theory))

---

**最后更新**：2024年
**维护者**：项目团队
