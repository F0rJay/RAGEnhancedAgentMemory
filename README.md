...
**RAGEnhancedAgentMemory** 是一个专为 AI Agent 设计的长对话记忆增强插件，通过分层记忆架构和语义检索技术，帮助您的 Agent 在长对话场景中保持上下文连贯性，避免信息遗忘和推理退化。

### 为什么需要这个插件？

在长对话场景中，传统 Agent 面临三大核心挑战：

| 问题 | 影响 | 我们的解决方案 |
|------|------|--------------|
| **记忆过载** | 滑动窗口导致早期关键信息丢失，Agent 无法记住对话开始时的设定 | ✅ 分层记忆架构：瞬时→短期→长期，自动归档重要信息 |
| **信息遗忘** | 全量摘要耗尽上下文窗口，无法有效检索历史对话 | ✅ 向量数据库语义检索：基于语义相似度快速定位相关历史 |
| **推理退化** | 海量历史数据线性查找效率低，响应延迟增加 | ✅ vLLM 加速推理：延迟降低 95.5%，吞吐量提升 80.2%（本地模型模式） |
